...
class ABDetLocalVisualizer(Visualizer):
    ...
    def _draw_instances(self, image: np.ndarray, instances: ['InstanceData'],
        ...
        if 'bboxes' in instances and instances.bboxes.sum() > 0:
            ...
            # Add loss_weight to the label text if it exists
                if 'loss_weight' in instances:
                    loss_weight = instances.loss_weight[i]
                    label_text += f' (w: {loss_weight:.2f})'

                self.draw_texts(
                    label_text,
                    pos,
                    colors=text_colors[i],
                    font_sizes=5,  # int(13 * scales[i]),
                    bboxes=[{
                        'facecolor': 'black',
                        'alpha': 0.8,
                        'pad': 0.7,
        ...
        if 'masks' in instances:
            ...
            if len(labels) > 0 and ('bboxes' not in instances or instances.bboxes.sum() == 0):
                ...
                # Add loss_weight to the label text if it exists
                    if 'loss_weights' in instances:
                        loss_weight = instances.loss_weights[i]
                        label_text += f' (w: {loss_weight:.2f})'
